...
SurfingSA::SurfingSA(bb::cascades::Application *app, const QUuid &uuid)
: QObject(app)
, m_context(uuid)
, m_isAllowed(false)
...
void SurfingSA::registerApplication() {
...
void SurfingSA::processRegistrationStatus(const RegistrationState::Type state) {
...
void SurfingSA::registrationFinished() {
...
void SurfingSA::dialogFinished(bb::system::SystemUiResult::Type value) {
	Application::exit(-1);
}

void SurfingSA::sendInvite() {
    if (!m_messageService) {
